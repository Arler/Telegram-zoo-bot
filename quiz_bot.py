...
class QuizBot(Quiz, TeleBot):
	...
	def __send_result(self, callback):
		...
		message = f"""\
			Моё тотемное животное из викторины **Название зоопарка** это {self.result.name}, хочешь узнать своё?
			Ты тоже можешь пройти викторину в телеграм боте **Название зоопарка**\
